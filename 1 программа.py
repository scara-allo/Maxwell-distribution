...
def open_redactor():
    global num_molecules, box_size, speed_limit

    window = tk.Toplevel()
    window.title("Редактор переменных")

    var1_str = tk.StringVar(value=str(num_molecules))
    var2_str = tk.StringVar(value=str(box_size))
    var4_str = tk.StringVar(value=str(speed_limit))

    # Поля для ввода
    tk.Label(window, text="количество молекул").grid(row=0, column=0)
    var1 = tk.Entry(window, textvariable=var1_str)
    var1.grid(row=0, column=1)

    tk.Label(window, text="размер области").grid(row=1, column=0)
    var2 = tk.Entry(window, textvariable=var2_str)
    var2.grid(row=1, column=1)

    tk.Label(window, text="ограничение скорости").grid(row=2, column=0)
    var4 = tk.Entry(window, textvariable=var4_str)
    var4.grid(row=2, column=1)

    def save():
        global num_molecules, box_size, speed_limit
        try:
            num_molecules = int(var1_str.get())
            box_size = int(var2_str.get())
            speed_limit = int(var4_str.get())
        except ValueError:
            print ('Введите числа')
        window.destroy()
